...
gcloud run deploy $SERVICE   --image gcr.io/$PROJECT_ID/$SERVICE:latest   --service-account=rag-svc-ac@btoproject-486405.iam.gserviceaccount.com   --region=$REGION   --set-env-vars PROJECT_ID=$PROJECT_ID,REGION=$REGION,VERTEX_LOCATION=$REGION,VERTEX_INDEX_ID=projects/btoproject-486405/locations/us-central1/indexes/rag-index,VERTEX_INDEX_ENDPOINT=projects/btoproject-486405/locations/us-central1/indexEndpoints/PLACEHOLDER,DEPLOYED_INDEX_ID=rag-index-deployed,MODEL_VARIANT=gemini-2.0-flash-001   --allow-unauthenticated
